...
validateInsufficientGAInsightsForHomepage=exports.validateInsufficientGAInsightsForHomepage = async function () {
    //find no tile content text
    await common.delay(5000);
    await validateElementToIncludeTextByClassName("tile-no-content__text",
        "Looks like there’s not quite enough data for us to recommend content ideas."
    );
};
validateNoGACompetitorForHomepage=exports.validateNoGACompetitorForHomepage = async function () {
    //find no tile content text
    await common.delay(5000);
    await validateElementToIncludeTextByClassName("tile-no-content__text",
